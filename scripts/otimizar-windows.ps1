...
if ($Headless -and $Acao) {
    ...
    try {
        switch ($Acao) {
            ...
            "90" { Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\PolicyManager\default\WiFi\AllowWiFiHotSpotReporting" -Name "value" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\PolicyManager\default\WiFi\AllowWiFiSense" -Name "value" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\WcmSvc\wifinetworkmanager\config" -Name "AutoConnectAllowedOEM" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\WcmSvc\wifinetworkmanager\config" -Name "AutoConnectAllowedUser" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Write-Host "[OK] Wi-Fi Sense desativado" -ForegroundColor $script:c.Green; Log-Tweak "Privacidade" "Desativou" "Wi-Fi Sense" }
            "91" { Set-ItemProperty -Path "HKCU:\Software\Microsoft\Speech\Preferences" -Name "VoiceActivationEnabled" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Holographic" -Name "VoiceActivationEnabled" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue; Write-Host "[OK] Ativacao por voz desativada" -ForegroundColor $script:c.Green; Log-Tweak "Privacidade" "Desativou" "Ativ. Voz" }
            "92" { $hostsPath = "$env:SystemRoot\System32\drivers\etc\hosts"; $telemetryHosts = @("0.0.0.0 vortex-win.data.microsoft.com","0.0.0.0 settings-win.data.microsoft.com","0.0.0.0 telemetry.microsoft.com","0.0.0.0 telemetry.appex.bing.net","0.0.0.0 telemetry.urs.microsoft.com","0.0.0.0 df.telemetry.microsoft.com","0.0.0.0 oca.telemetry.microsoft.com","0.0.0.0 sqm.telemetry.microsoft.com","0.0.0.0 watson.telemetry.microsoft.com","0.0.0.0 vortex-sandbox.data.microsoft.com","0.0.0.0 v10.vortex-win.data.microsoft.com","0.0.0.0 watson.microsoft.com","0.0.0.0 watson.live.com","0.0.0.0 watson.ppe.telemetry.microsoft.com","0.0.0.0 vortex.data.microsoft.com","0.0.0.0 preview.msn.com","0.0.0.0 reports.wes.df.telemetry.microsoft.com","0.0.0.0 services.wes.df.telemetry.microsoft.com"); $content = Get-Content $hostsPath -Raw -ErrorAction SilentlyContinue; foreach ($entry in $telemetryHosts) { if ($content -notmatch [regex]::Escape($entry)) { Add-Content -Path $hostsPath -Value $entry -Force -ErrorAction SilentlyContinue } }; Write-Host "[OK] Hosts de telemetria bloqueados" -ForegroundColor $script:c.Green; Log-Tweak "Privacidade" "Bloqueou" "Hosts Telemetria" }
            "93" { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU" -Name "NoAutoUpdate" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Set-Service -Name wuauserv -StartupType Disabled -ErrorAction SilentlyContinue; Stop-Service -Name wuauserv -Force -ErrorAction SilentlyContinue; Write-Host "[OK] Windows Update desativado (so para maquinas isoladas)" -ForegroundColor $script:c.Red; Log-Tweak "Privacidade" "Desativou" "Windows Update" }
            "94" { Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "BlockDomainPicture" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "DontDisplayLastUsername" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Write-Host "[OK] Opcao de Microsoft Account removida" -ForegroundColor $script:c.Green; Log-Tweak "Privacidade" "Removeu" "MS Account" }
            "95" { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender" -Name "DisableAntiSpyware" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection" -Name "DisableRealtimeMonitoring" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Set-Service -Name WinDefend -StartupType Disabled -ErrorAction SilentlyContinue; Stop-Service -Name WinDefend -Force -ErrorAction SilentlyContinue; Write-Host "[OK] Windows Defender desativado (tenha outro antivirus ativo)" -ForegroundColor $script:c.Red; Log-Tweak "Privacidade" "Desativou" "Defender" }
