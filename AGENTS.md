...
2. Copiar `scripts/otimizar-windows.ps1` para `build/publish/scripts/` (e `icon.ico` + `recursos/logos/*.png` para `build/publish/`).
3. Regerar o instalador Inno Setup (`implantacao/tloptimizer.iss`) → `build/installer/TLOptimizer-Setup-X.Y.Z.exe`; sincronizar `dist/instalador/` e `dist/publicacao/` via `scripts/build-all.ps1`.
4. Subir a versão em `implantacao/tloptimizer.iss` (#define MyAppVersion), em `implantacao/update-manifest.json` ("version"), no `src/TLOptimizer.Launcher/AppConfig.cs` (LauncherVersion) e registrar no changelog/notes — tudo junto.
...
- MANTER `dist/` sempre limpo: apenas `instalador/` (instalador único Inno single-file `TLOptimizer-Setup-X.Y.Z.exe` — o instalador WPF antigo foi descontinuado) e `publicacao/` (launcher).
...
- APÓS qualquer alteração no `implantacao/tloptimizer.iss` ou no código do launcher, rodar `scripts/build-all.ps1` para regenerar o instalador e sincronizar `dist/` (nunca esperar o usuário pedir). O usuário testa diretamente o executável em `dist/instalador/`.
